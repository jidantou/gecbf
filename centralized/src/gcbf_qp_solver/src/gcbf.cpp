...
int Gcbf::computeQPCoefficientsAl(Eigen::SparseMatrix<double>& A, Eigen::VectorXd& l) {
    if (!log_opened_) {
        ...
        std::string log_path = std::string(home) + "/Projects/gcbf_log/gcbf_constraints_h.csv";
        log_file_.open(log_path, std::ios::out);
        if (log_file_.is_open()) {
            ROS_INFO("GCBF constraints log file opened: %s", log_path.c_str());
            // 写入表头：timestamp,constraint_index,type,drone_i,drone_j,obstacle_idx,h_value
            log_file_ << "timestamp,constraint_idx,type,drone_i,drone_j,obstacle_idx,h_value\n";
            log_opened_ = true;
        } else {
    ...
    get_neighbors();
    _all_constraints.clear();

    for (int i = 0; i < _DRONE_NUM; i++)
    {
        compute_dynamic_f(i, f[i]);
        compute_dynamic_g(i, g[i]);
        get_all_constraints_for_drone(i);
    }

    // log h_value for all constraints
    int constraint_idx = 0;
    for (const auto& con : _all_constraints) {
        // 推断约束类型：通过gradients数量等判断（障碍物约束只有一个grad，无人机约束有两个）
        std::string type = (con.gradients.size() == 1) ? "obstacle" : "drone";
        int drone_i = -1, drone_j = -1, obs_idx = -1;

        if (con.gradients.size() >= 1) {
            drone_i = con.gradients[0].first;
        }
        if (con.gradients.size() >= 2) {
            drone_j = con.gradients[1].first;
        }

        // 障碍物索引可以在此处记录（需要在生成约束时保留obs索引，但现有代码未保留，可暂用-1）
        // 若有需要可修改 get_all_constraints_for_drone 保存障碍物编号
        obs_idx = con.obstacle_index;

        if (log_file_.is_open()) {
            log_file_ << std::fixed << std::setprecision(9) 
                      << current_time << "," << constraint_idx << "," 
                      << type << "," << drone_i << "," << drone_j << "," 
                      << obs_idx << "," << con.h_value << "\n";
        }
        constraint_idx++;
    }

    int num_constraints = _all_constraints.size();
    if (num_constraints == 0)
        num_constraints = 1;
    int num_vars = 3 * _DRONE_NUM;
    A.resize(num_constraints, num_vars);
    l.resize(num_constraints);

    std::vector<Eigen::Triplet<double>> triplets;
    triplets.reserve(_DRONE_NUM * 8);

    bool hLessZero = false;
    for (int row = 0; row < _all_constraints.size(); ++row) {
        const auto& con = _all_constraints[row];
        double sum_f_grad = 0.0;

        for (const auto& grad_pair : con.gradients) {
            int drone_id = grad_pair.first;
            const PartStateType& grad = grad_pair.second;
            Eigen::Vector3d temp = grad.TransposeDot(g[drone_id]);
            for (int k = 0; k < 3; ++k) {
                triplets.emplace_back(row, drone_id * 3 + k, temp(k));
            }
            sum_f_grad += grad.TransposeDot(f[drone_id]);
        }
        l(row) = -_alpha0 * con.h_value - sum_f_grad;

        // DEBUG
        // if (l(row) > 0) {
        //     ROS_INFO("%d: l(%d) > 0: l(%d):%.2f, h_value:%.2f, sum_f_grad:%.2f", debug_count, row, row, l(row), con.h_value, sum_f_grad);
        // }
        ...
        if (con.h_value < 0) {
            ...
            char s[100];
            snprintf(s, 100, "h_value < 0! %d: h_value: %.3f, gradents:\n", debug_count, con.h_value);
            oss << s;
            for (auto &&temp_debug : con.gradients)
            {
    ...
    if (hLessZero) {
        ...
        for (int i = 0; i < num_constraints; ++i)
        {
            ...
            row_ss << "    A[" << i << "]: [";
            for (int j = 0; j < 3 * _DRONE_NUM; ++j)
            {
                row_ss << std::setw(5) << A.coeff(i, j);  // 宽度10，右对齐
                if (j < 3 * _DRONE_NUM - 1) row_ss << ", ";
            }
...
void Gcbf::get_all_constraints_for_drone(int drone_id) {
    const Eigen::Vector3d& p_i = _drones_state_ptr[drone_id].pos;
    const Eigen::Vector3d& v_i = _drones_state_ptr[drone_id].vel;

    // 1. 障碍物约束（每个障碍物单独一个约束）
    const std::vector<Eigen::Vector3d>& obs = _local_maps[drone_id].getVisibleSpheres();
    // const auto& obs_indices = _local_maps[drone_id].getOccupiedVoxelIndices();
    for (size_t idx = 0; idx < obs.size(); ++idx) {
        // Eigen::Vector3d o_j = _local_maps[drone_id].getVoxelCenter(obs_indices[idx]);
        double h_val;
        Eigen::Vector3d grad_p, grad_v;
        compute_h_j_obstacle(obs[idx], p_i, v_i, h_val, grad_p, grad_v);
        ConstraintInfo info;
        info.h_value = h_val;
        PartStateType grad_i;
        grad_i.pos = grad_p;
        grad_i.vel = grad_v;
        info.gradients.emplace_back(drone_id, grad_i);
        info.obstacle_index = idx;  // 记录障碍物索引
        _all_constraints.push_back(info);
    }

    // 2. 其他无人机约束（仅邻居，且跳过自身）
    const auto& nb = _neighbors[drone_id];
    for (int j : nb) {
        if (j == drone_id) continue;
        const Eigen::Vector3d& p_j = _drones_state_ptr[j].pos;
        const Eigen::Vector3d& v_j = _drones_state_ptr[j].vel;
        double h_val;
        Eigen::Vector3d grad_pi, grad_vi, grad_pj, grad_vj;
        ...
        ConstraintInfo info;
        info.h_value = h_val;
        PartStateType grad_i, grad_j;
        grad_i.pos = grad_pi;
        grad_i.vel = grad_vi;
        grad_j.pos = grad_pj;
        grad_j.vel = grad_vj;
        info.gradients.emplace_back(drone_id, grad_i);
        info.gradients.emplace_back(j, grad_j);
        _all_constraints.push_back(info);
    }
...
void Gcbf::closeLogFile() {
    if (log_file_.is_open()) {
        log_file_.flush();
        log_file_.close();
        ROS_INFO("GCBF constraints log file closed.");
    }
}
