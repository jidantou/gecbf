...
struct MappingData
{
  ...
  std::vector<Eigen::Vector3i> occupied_voxel_indices_;               // 存储所有占用体素的索引
  std::unordered_map<Eigen::Vector3i, size_t> occupied_voxel_index_map_; // 索引到 vector 位置的映射
...
public:
  ...
  void initMap(ros::NodeHandle &nh, int drone_id);
  ...
  // 返回所有占用体素的索引（常量引用，避免拷贝）
  const std::vector<Eigen::Vector3i>& getOccupiedVoxelIndices() const { return md_.occupied_voxel_indices_; }
  // 根据体素索引计算中心点坐标
  inline Eigen::Vector3d getVoxelCenter(const Eigen::Vector3i& idx) { return globalIdx2Pos(idx); }
...
private:
  ...
  void addOccupiedVoxel(const Eigen::Vector3i& idx);
  void removeOccupiedVoxel(const Eigen::Vector3i& idx);
