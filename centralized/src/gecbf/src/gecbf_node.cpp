...
bool correction(ros::NodeHandle& nh, const int drone_num, Eigen::VectorXd& solution, const std::vector<Eigen::Vector3d>& ref_ctrls, const Eigen::SparseMatrix<double>& A, const Eigen::VectorXd& l);
...
int main(int argc, char** argv) {
    ...
    ROS_INFO("Waiting for other nodes to start...");
    ros::Duration(2.0).sleep();  // wait for other nodes to start
    ...
    while (ros::ok()) {
        ...
        if (solver.solveProblem() == OsqpEigen::ErrorExitFlag::NoError) {
            ...
            if (count_debug % 200 == 0) {
                ...
                const double active_threshold = 1e-3;
                ...
                for (int i = 0; i < rows; ++i) {
                    ...
                    ss << " ]";
                    // 输出 l 的对应元素（右侧列向量形式）
                    const bool is_active = (ax(i) - l(i)) <= active_threshold;
                    GECBF_QP::ConstraintInfoType& conInfo = gecbf_solution.debug_cons_[i];
                    if (i == 0) {
                        ss << "   l = [ " << l(i) << " (" << (is_active ? "Active" : "Deactivate") << ")";
                        if (conInfo.type == GECBF_QP::OBSTACLE) {
                            ss << " O" << conInfo.current_drone_id << ": " << conInfo.o_j.x() << "," << conInfo.o_j.y() << "," << conInfo.o_j.z();
                        } else if (conInfo.type == GECBF_QP::DRONE) {
                            ss << " D" << conInfo.current_drone_id << ": " << conInfo.other_drone_id;
                        }
                    } else if (i == rows - 1) {
                        ss << "        " << l(i) << " (" << (is_active ? "Active" : "Deactivate") << ") ]";
                        if (conInfo.type == GECBF_QP::OBSTACLE) {
                            ss << " O" << conInfo.current_drone_id << ": " << conInfo.o_j.x() << "," << conInfo.o_j.y() << "," << conInfo.o_j.z();
                        } else if (conInfo.type == GECBF_QP::DRONE) {
                            ss << " D" << conInfo.current_drone_id << ": " << conInfo.other_drone_id;
            ...
            bool cor_res = false;
            cor_res = correction(nh, drone_num, solution, ref_ctrls, A, l);

            // DBEUG
            if (count_debug % 50 == 0) {
                ROS_INFO("%d:  correction result: %s", count_debug, cor_res ? "success" : "no correction");
            }
            // if (cor_res) {
            //     ROS_INFO("%d: hsa corrected!", count_debug);
            // }
...
bool correction(ros::NodeHandle& nh, const int drone_num, Eigen::VectorXd& solution, const std::vector<Eigen::Vector3d>& ref_ctrls, const Eigen::SparseMatrix<double>& A, const Eigen::VectorXd& l) {
    double cos_threshold = -0.7;  // 设定一个阈值，判断是否需要修正
    double vel_threshold = 0.5;  // 设定一个速度阈值，判断无人机是否接近静止
    double gravity_compensation = 9.81 * 0.98;  // 重力补偿值
    double correction_force_magnitude = 1.0;  // 修正力的大小，可以根据需要调整
    ...
    for (int i = 0; i < ref_ctrls.size(); i++) {
        ...
        if (cos_value < cos_threshold && states[i].vel.norm() < vel_threshold) {
            ...
            if (std::abs(f_ref.z()) < 0.05) {
                z_e = Eigen::Vector3d(0, 0, 1);
                x_e = Eigen::Vector3d(1, 0, 0);
                y_e = Eigen::Vector3d(0, 1, 0);
            } else {
                z_e = f_ref.normalized();
                x_e = z_e.cross(Eigen::Vector3d(0, 0, 1));
                x_e.normalize();
                y_e = z_e.cross(x_e);
            }

            double corr_coeff = correction_force_magnitude;
            for (int iter = 0; iter < 20; iter++) {  // 限制迭代次数
                ...
                for (int theta_iter = 0; theta_iter < 10; theta_iter++) {
                    ...
                    theta = i * ((2 * M_PI) / drone_num) + theta_iter * ((2 * M_PI) / 10);
